...
for query in baseline["queries"]:
    ...
    # Check if any kept action touched files related to this query's source
...
# Collect all files changed by kept actions (strategy is free-text, not enum)
for k in autoloop_state["kept"]:
